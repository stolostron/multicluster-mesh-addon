...
.PHONY: deploy
deploy: gen push ## Deploy the controller to the cluster (PLATFORM=openshift|kind)
	kubectl apply -f config/crd/
	kubectl kustomize config/deploy/overlays/$(PLATFORM) | sed 's|image:.*|image: $(IMG)|' | kubectl apply -f -
...
undeploy: gen ## Remove the controller from the cluster (PLATFORM=openshift|kind)
	...
	kubectl kustomize config/deploy/overlays/$(PLATFORM) | kubectl delete -f - --ignore-not-found=true
