...
LDFLAGS := -X github.com/stolostron/multicluster-mesh-addon/pkg/version.buildVersion=$(VERSION)
LDFLAGS += -X github.com/stolostron/multicluster-mesh-addon/pkg/version.buildGitRevision=$(GIT_COMMIT)
...
.PHONY: gen-chart
gen-chart: ## Update Chart.yaml and values.yaml with VERSION
	@sed -i "s/^version:.*/version: $(VERSION)/" $(CHART_DIR)/Chart.yaml
	@sed -i "s/^appVersion:.*/appVersion: v$(VERSION)/" $(CHART_DIR)/Chart.yaml
	@sed -i "s/^  tag:.*/  tag: $(TAG)/" $(CHART_DIR)/values.yaml

.PHONY: gen
gen: gen-code gen-crds gen-rbac gen-chart ## Generate code, CRDs, and RBAC

.PHONY: verify-gen
verify-gen: gen ## Verify generated files are up to date
	@if ! git diff --quiet HEAD -- chart/crds/ chart/templates/clusterrole.yaml chart/Chart.yaml chart/values.yaml pkg/apis/**/zz_generated.*.go; then \
...
verify-istio-reader-rbac: ## Verify istio-reader ClusterRole/ClusterRoleBinding match upstream Istio (requires network; set ISTIO_READER_SKIP=1 to skip)
	@echo "Verifying istio-reader RBAC against upstream Istio ($(or $(ISTIO_READER_REF),master))..."
	@go test ./pkg/hub/mesh/ -run '^TestIstioReaderRBACMatchesUpstream$$' -count=1 -timeout 2m
...
deploy: gen push $(HELM_BIN) ## Deploy the controller using Helm (PLATFORM=openshift|kind)
	$(HELM) upgrade --install $(HELM_RELEASE_NAME) $(CHART_DIR) \
		...
		--set image.repository=$(HUB)/$(IMAGE_BASE) \
		--set image.tag=$(TAG) \
